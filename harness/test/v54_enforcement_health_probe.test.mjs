...
const PINNED_MATRIX_DIGEST = "sha256:5e0d9ffc70735d6b98160fac932f73daa5527299be385426e4ec2e84e2271904";
...
function patchProbe(patch) {
  return { ...ACTIVE_HEALTH, probe: { ...ACTIVE_HEALTH.probe, ...patch } };
}

function patchLandlock(patch) {
  return { ...ACTIVE_HEALTH, landlock: { ...ACTIVE_HEALTH.landlock, ...patch } };
}
...
test("executeLocalProducer refuses well-formed but invalid producer records", () => {
  ...
  try {
    ...
    const cases = [
      ...
      ["blocked_port banana", patchProbe({ blocked_port: "banana" }), /blocked_port|allowlist|denied/i],
      ["blocked_port 0", patchProbe({ blocked_port: 0 }), /blocked_port|allowlist|denied/i],
      ["blocked_port 99999", patchProbe({ blocked_port: 99999 }), /blocked_port|allowlist|denied/i],
...
test("committed matrix pins the hosted v5.4.0 enforcement-health fold", () => {
  ...
  assert.equal(matrix.generated.last_verified_assay, "v6.0.0");
  ...
  assert.match(raw, /"digest": "sha256:5e0d9ffc70735d6b98160fac932f73daa5527299be385426e4ec2e84e2271904"/);
});
