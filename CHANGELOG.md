...
- compatibility: add Trust Card compatibility probe and validator machinery (#205). Implements pure structural validator `validateTrustCardCompatibility` for canonical `trustcard.json` artifacts, checking schema version 5, all 10 frozen claim identities, known enum vocabularies, frozen non-goals, and claim parity against paired Trust Basis artifacts. Wires bounded measurement script `harness/scripts/probe-trust-card-compat.mjs` into the `assay-release-compatibility` CI job following receipt recipes. Hosted proof remains pending until a hosted run is executed and retained. Preserves historical v5.4.0 enforcement-health and v6.0.0 receipt recipe rows.
