...
- compatibility: record digest-bound Assay v6.0.0 three-family receipt compatibility rail from hosted run `33957799594` (#205). Adds sibling release-compatibility recipe row with artifact digest `sha256:cac2d6b3…` matching the uploaded Actions artifact; binds release peel `7df13b3f…` and Linux archive digest `sha256:71f0854b…`. Preserves historical `v3.27.0` release-compatibility rail, `v3.29.0` supply-chain recipe provenance, and unrefreshed `v5.4.0` enforcement-health carrier row. Emitted `trust-basis.json` carries claims without `schema_version = 5`, and binary digest is not recorded in artifact; these residual producer/reporting gaps remain visible. Not a claim of universal host compatibility, Darwin/aarch64 execution, or enforcement-health refresh.
...
- compatibility: add Trust Card compatibility probe and validator machinery (#205). Implements pure structural validator `validateTrustCardCompatibility` for canonical `trustcard.json` artifacts, checking schema version 5, all 10 frozen claim identities, known enum vocabularies, frozen non-goals, and intra-binary claim parity against paired Trust Basis artifacts (measuring same-producer consistency with single degree of freedom `level`). Enforces exact released `note: string | null` type shape and symmetric paired-basis rejection of extra keys. Hardens probe with copy-first bounded input retention, pre/post input digest invariance, refusal of pre-existing outputs for fresh output identity, and strict CLI option/ceiling validation. Wires bounded measurement script `harness/scripts/probe-trust-card-compat.mjs` into the `assay-release-compatibility` CI job following receipt recipes, protected by structural AST workflow contract tests with isolating mutation controls. Hosted proof remains pending until a hosted run is executed and retained. Preserves historical v5.4.0 enforcement-health and v6.0.0 receipt recipe rows.
...
- promotion: freeze `probe-policy.yaml` as a load-bearing enforcement-health measurement input (#193/#195). A policy edit after the hosted run now fails closed as `measurement_surface_changed`; this does not add a new hosted proof or change matrix rows.
- probe: list the v5.4 archive from a parent-opened read-only fd (stdio fd 3) so the listing helper never opens an argv path (#184 / CodeQL `js/path-injection`). Missing fd 3 refuses; there is no path fallback.
- probe: bound local v5.4 extract before materialization and close the package deep-import of `scripts/probe-v54-enforcement-health.mjs` (#184). `executeLocalProducer` now refuses expanded-byte / entry overflow, symlink/hardlink types, and timed-out listing before `tar -xzf`. Ceilings are 32 MiB / 8 entries, above the measured published x86_64 layout (31,683,222 ISIZE bytes, 4 entries). `package.json` `exports`/`files` advertise `dist/cli.js` and omit the probe script from the packed surface. This is formally deep-import-breaking; there are zero known consumers and `assay-harness` is not on the public registry on this line. Not a claim the published v5.4.0 archive is malicious or oversized, and not a claim `dist/` already ships in today's pack. The frozen probe script changes, so a later promotion needs a fresh hosted proof.
- contracts: wire the enforcement-health promotion verifier's committed-copy check through production CLI deps (#188). `readCommitted` reads the promoting head via `git show` of `harness/fixtures/suite-compatibility/enforcement-health/{enforcement-health.json,recipe.provenance.json}` and fail-closes `artifact_changed` when those bytes differ from the downloaded carrier/provenance. Git/show errors and a null committed read are `api_failure`, not a skip. Working-tree bytes are not accepted. No new GitHub contents/raw route. Not a new proof, host-support, or current-support claim.
- contracts: fold `assay.enforcement_health.v1` to hosted `end_to_end=proven` from `Harness CI` run `33080407473` (#168 Phase B). The shared carrier rule now fail-closes `proven` plus `end_to_end_gap`, missing hermetic proof fields, `proof_scope.ambient_scan=true`, `proof_scope.hosted=false`, or `proof.runner_os != proof_scope.runner_os`. `artifact_digest` is the carrier JSON (`5aa7513e…`), not the Actions zip or the published tarball; those swaps fail in the promotion verifier. `generated.last_verified_assay` / deprecated `assay_default` become `v5.4.0`; `verified_on` stays `2026-06-17`; `harness_version` stays `0.10.2`. Not a universal-host claim, not an aarch64 measurement, and not a current-support range.
- contracts: revalidate the Assay v5.4.0 Trust Basis / Trust Card / receipt contract and add a digest-bound local-asset `assay.enforcement_health.v1` probe (#168 PR2b). Schema id is the measured peel string (`assay.enforcement_health.v1`, underscore). The CLI pins the published Linux CLI digest, snapshots the asset while hashing, refuses caller `--workdir` and pre-existing `--out`, and allows ceilings only to be lowered. Printed `active` requires the measured Landlock invariant plus a real-block probe; a missing `landlock.allowed_connect_tcp_ports` fails closed. Owned probe temp dirs are removed on success and failure; caller `--out` is preserved. Matrix rows, proofs, `generated`, and `manifest.digest` stay frozen. Not a hosted Linux runtime proof and not a current-support claim.
- contracts: `suite generate` write-mode serialization failures stay artifact-contract (3); an actual filesystem output write failure after successful serialization is `ci_formatter` (7), matching every other write-capable Harness verb. Malformed JSON is artifact-contract (3) with one routed diagnostic and no rewrite (#168).
...
- contracts: add a consumer-side action evidence index validator and a validation-if-present CI job (#177). Bounded index read, stream hashes, canonical POSIX path identity, and explicit NUL/backslash/drive-alias refuse. Workspace-bound index path, per-bundle/aggregate ceilings, production `--if-present`, structural workflow guard, canonical symlink identity, and component-aware `..`. Workflow parse uses pinned harness `js-yaml` 5.2.2 via `setup-node-harness`; index paths reject every symlink component. Per-bundle ceiling is 100 MiB; aggregate is an independent 1 GiB fail-closed bound (may refuse a producer-legal 100-row maximum). Absolute `index_path` is refused before open; the producer contract is workspace-relative (`.assay-reports/evidence-index.json`). Require slice for the dedicated live `action-evidence-index` job only: drop `--if-present` and pass `--require-verified` so absence/empty outputs and `evidence_state != verified` fail closed. This is the only validator callsite; there is no later require-slice. `sandbox-command: true` proves index/bundle integrity only (not a whole-action verdict). A future Windows producer can hard-fail via assay-action#37 (Windows drive-letter / index_abs); that is a non-claim, not a code path. Other jobs, schemas, and the Action pin are unchanged. `--if-present` and `--require-verified` are incompatible and fail before the all-empty return.
...
- Added the **Evidence Pack** (`suite.evidence_pack.v0`, H-next-3): a deterministic,
  ...
  (`suite.recipe_provenance.v0`) by digest — it creates no new evidence, approves no policy,
  and does not replace Plimsoll (VSA-shaped, not a SLSA VSA). v0 carries the proven inventory
