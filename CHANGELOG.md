...
- contracts: revalidate the Assay v5.4.0 Trust Basis / Trust Card / receipt contract and add a digest-bound local-asset `assay.enforcement_health.v1` probe (#168 PR2b). Schema id is the measured peel string (`assay.enforcement_health.v1`, underscore). Matrix rows, proofs, `generated`, and `manifest.digest` stay frozen. Not a hosted Linux runtime proof and not a current-support claim.
